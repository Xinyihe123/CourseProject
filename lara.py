...
def helper(reviewDataList, itemList):
  reviewMap = {}
  allTerms, curr, curr_freq, reviews, titles, ratings = [], [], [], [], [], [], []
  for i in range(len(reviewDataList)):
    for review in reviewDataList[i]['Reviews']:
  ...
  return vocab, cnt, vocabDict, curr, curr_freq, reviews, reviewMap, titles, ratings, tmp

def initializeParameters(k, V, N):
    mu = np.random.random_sample(k) # prior for \alpha in each review
    beta = np.ones([k,len(vocabDict)])
    ...
    eta = np.ones([N, k])
    gamma = np.ones([N, k])
    epsilon = np.random.random_sample([k, len(vocabDict)])
    epsilon =  normalize(epsilon)
    for d in range(0, N):
        L = len(reviewFreqDictList[d])
...
def runEM(r, W):
    k = 7
    V = len(vocabDict)
    N = len(reviewFreqDictList)
    mu, beta, sigma_inv, sigma, detla, alpha, s, phi,eta, gamma,epsilon, sigmaSquare= initializeParameters(V, k)
    converge = False
    iteration = 100
    old = calc_log(r, alpha, beta, W,delta, mu, sigma, vocabDict)
    while iteration > 0 or not converge:
        alpha, s,new_phi = E_step(mu, sigma, delta, beta, alpha, W, r, vocabDict, sigma_inv)
        mu, beta, delta, sigma = M_step(mu, sigma, delta, beta, alpha, W,r, vocabDict, sigma_inv, s)
        loglikelihood = calc_log(r, alpha, beta, W,delta, mu, sigma, vocabDict)
...
for file in os.listdir(folder):
    if file.endswith('.json'):
      with open(folder + '/' + file, encoding='utf-8') as data_file:
        reviewDataList.append(json.load(data_file))
        itemList.append(file.split('.')[0])
itemList, reviewDataList = getData('result/')
# vocab, cnt, vocabDict, reviewList, curr_freq, reviews, reviewMap, titles, ratings, tmp = helper(reviewDataList, itemList)
  
for k, v in reviewMap.items():
  overall_rating = np.array(ratings)
  W = curr_freq
  word_index = vocabDict
  s = runEM(ratings, W)
